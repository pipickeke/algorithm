...
public class Peaks_in_Array_4 {

    public static void main(String[] args) {
        int[] nums = {3,1,4,2,5};
        int[][] queries = {{2,3,4},{1,0,4}};
        Peaks_in_Array_4 bean = new Peaks_in_Array_4();
        List<Integer> ans = bean.countOfPeaks(nums, queries);
        System.out.println(ans);
    }

    public List<Integer> countOfPeaks(int[] nums, int[][] queries){
        int N = nums.length;
        Fenwick_5 f = new Fenwick_5(N - 1);
        for (int i = 1; i < N - 1; i++) {
            update(f, nums, i, 1);
        }
    ...
    public void update(Fenwick_5 f, int[] nums, int index, int val){
